...
pub enum InvalidIdentity {
    ...
    MissingAt,
    #[error("agent component is empty")]
    EmptyAgent,
    #[error("device component is empty")]
    EmptyDevice,
    #[error("component contains invalid chars (allowed: a-z, 0-9, -, _)")]
    BadChars,
}
...
impl Identity {
    pub fn new(
        ...
        if agent.is_empty() {
            return Err(InvalidIdentity::EmptyAgent);
        }
        if device.is_empty() {
            return Err(InvalidIdentity::EmptyDevice);
        }
    ...
    pub fn git_author_line(&self) -> (String, String) {
        (
            self.to_string(),
            format!("{}@{}.stash", self.agent, self.device),
        )
    }
}
...
mod tests {
    ...
    fn rejects_empty_components() {
        assert_matches!(
            Identity::new("", "tootie"),
            Err(InvalidIdentity::EmptyAgent)
        );
        assert_matches!(
            Identity::new("claude", ""),
            Err(InvalidIdentity::EmptyDevice)
        );
        assert_matches!(Identity::parse("@tootie"), Err(InvalidIdentity::EmptyAgent));
        assert_matches!(
            Identity::parse("claude@"),
            Err(InvalidIdentity::EmptyDevice)
        );
    }
    ...
    fn rejects_invalid_chars() {
        assert_matches!(
            Identity::new("claude!", "tootie"),
            Err(InvalidIdentity::BadChars)
        );
        assert_matches!(
            Identity::new("claude", "too tie"),
            Err(InvalidIdentity::BadChars)
        );
    }
