...
impl StashRepo {
    pub async fn list(
        ...
        let filtered: Vec<_> = entries.into_iter()
            .filter(|e| matcher.as_ref()
                .map_or(true, |m| m.matches(&e.path)))
            .collect();

        let start = cursor.as_ref()
            .map(|c| c.0.parse::<usize>()
                .map_err(|_| StashError::InvalidInput {
                    field: "cursor".into(), reason: "not a number".into() }))
            .transpose()?
            .unwrap_or(0);
...
mod tests {
    ...
    async fn seed(r: &StashRepo, paths: &[&str]) {
        for p in paths {
            r.write(&StashPath::parse(p).unwrap(),
                    Bytes::from(format!("x-{p}")), &id(), None).await.unwrap();
        }
    }
    ...
    async fn list_at_prefix_returns_matching_files() {
        ...
        seed(&r, &["docs/a.md","docs/b.md","docs/sub/c.md","README.md"]).await;
        let page = r.list(
    ...
    async fn list_with_glob_filters_matches() {
        ...
        let page = r.list(
            &StashPath::parse("docs").unwrap(),
            None, Some("**/*.md"), None, 100).await.unwrap();
        let got: Vec<_> = page.entries.iter().map(|v| v.path.as_str()).collect();
    ...
    async fn list_paginates_with_stable_ordering() {
        ...
        let p1 = r.list(&StashPath::parse("d").unwrap(),
                        None, None, None, 2).await.unwrap();
        assert_eq!(p1.entries.len(), 2);
        let c1 = p1.next_cursor.clone().expect("cursor on first page");
        let p2 = r.list(&StashPath::parse("d").unwrap(),
                        None, None, Some(c1), 2).await.unwrap();
        assert_eq!(p2.entries.len(), 2);
        let c2 = p2.next_cursor.clone().expect("cursor on second page");
        let p3 = r.list(&StashPath::parse("d").unwrap(),
                        None, None, Some(c2), 2).await.unwrap();
