...
impl StashRepo {
    pub async fn diff(
        ...
        let unified = git_helpers::blocking(move || diff_blobs(&repo_path, &path_s, &from_c, to_c.as_ref())).await?;
        Ok(DiffText { unified })
    }
}
...
fn diff_blobs(
    ...
    if old == new { return Ok(String::new()); }
...
mod tests {
    ...
    async fn diff_shows_unified_output_between_commits() {
        ...
        let v1 = r.write(&p, Bytes::from("hello\n"), &id(), None).await.unwrap();
        let v2 = r.write(&p, Bytes::from("goodbye\n"), &id(), None).await.unwrap();
    ...
    async fn diff_empty_when_same_content() {
        ...
        let r = StashRepo::init(td.path()).await.unwrap();
        let p = StashPath::parse("x.md").unwrap();
        let v = r.write(&p, Bytes::from("one\n"), &id(), None).await.unwrap();
    ...
    async fn diff_to_head_when_to_omitted() {
        ...
        let r = StashRepo::init(td.path()).await.unwrap();
        let p = StashPath::parse("x.md").unwrap();
        let v1 = r.write(&p, Bytes::from("one\n"), &id(), None).await.unwrap();
