...
pub struct StashRepo {
    #[allow(dead_code)] root:        PathBuf,          // $STASH_ROOT
    repo_path:   PathBuf,          // $STASH_ROOT/repo.git
    #[allow(dead_code)] write_lock:  Arc<Mutex<()>>,
}
...
impl StashRepo {
    pub async fn init(root: impl AsRef<Path>) -> StashResult<Self> {
        ...
        std::fs::create_dir_all(&root)
            .map_err(|e| StashError::Internal { trace_id: format!("mkdir:{e}") })?;
        ...
        .await?;
        Ok(Self { root, repo_path, write_lock: Arc::new(Mutex::new(())) })
    }
...
mod tests {
    ...
    async fn open_errors_when_missing() {
        ...
        assert!(matches!(err, StashError::Internal { .. }),
