    ...
    pub async fn history(
        ...
        for e in entries {
            ...
            out.push(FileVersion {
                path: path.clone(),
                sha: e.blob_sha.unwrap_or_else(|| Sha::parse("0".repeat(40)).unwrap()),
                commit: e.commit_sha,
                size: e.size,
        ...
        let next = if has_more { Some(Cursor((skip + out.len()).to_string())) } else { None };
...
mod tests {
    ...
    async fn history_newest_first() {
        ...
        let v1 = r.write(&p, Bytes::from("one"), &id(), Some("1".into())).await.unwrap();
        let v2 = r.write(&p, Bytes::from("two"), &id(), Some("2".into())).await.unwrap();
        let v3 = r.write(&p, Bytes::from("three"), &id(), Some("3".into())).await.unwrap();
    ...
    async fn history_paginates() {
        ...
        let p = StashPath::parse("x.md").unwrap();
        for i in 0..4 {
            r.write(&p, Bytes::from(format!("{i}")), &id(), None).await.unwrap();
        }
